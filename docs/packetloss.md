...
1. Verify AWS identity, remote Terraform state, the current Route 53 records,
   existing GitHub OIDC provider, and budget recipient/scope. Review drift and
   import existing resources where required before applying.
...
3. Review the infrastructure changes, then run the manual Deployment workflow
   ...
   On a first deployment, publish the application immediately afterward; an
   empty site bucket cannot serve the game.
...
5. Obtain `packetloss_environments` from Terraform outputs. Check each API
   ...
   `/dev/assets` and `/dev/assets/`. Verify both custom domains and their HTTPS
   certificates. Confirm root HTML has no `/prod`
   refresh or redirect. Browser QA requires a separate explicit request.
6. Verify the budget amount, scope, and all notification subscribers using the
...
Keep the buckets and distributions provisioned during application recovery;
buckets reject destruction while nonempty. DNS continues to point to CloudFront.
